...
db_connection = sqlite3.connect('./efficientNet_similarity_scores.db')
...
def insert_similarity_score(id1, id2, score):
    # Insert similarity score into the database
    db_cursor.execute("INSERT OR REPLACE INTO similarity_scores (id1, id2, score) VALUES (?, ?, ?)", (id1, id2, score))
    db_connection.commit()
        ...
        for x in range(len(feature_vectors1)):
            for y in range(len(feature_vectors1)):
                if feature_vectors1[x] is not None and feature_vectors1[y] is not None:
                    ...
                    insert_similarity_score(all_ids[start_idx + x], all_ids[start_idx + y], sim)
            ...
            for x in range(len(feature_vectors1)):
                for y in range(len(feature_vectors2)):
                    if feature_vectors1[x] is not None and feature_vectors2[y] is not None:
                        ...
                        insert_similarity_score(all_ids[start_idx + x], all_ids[start_idx1 + y], sim)
